...
{
    public class StrTransactionService : IStrTransactionService
    {
        private readonly IStrTransactionRepository _repo;
        private readonly IStrTransactionList _list;
        public StrTransactionService(IStrTransactionRepository repo, IStrTransactionList list)
        {
            _repo = repo;
            _list = list;
        }

        public async Task<StrTransaction?> GetAsync(long id)
        {
            var result = await _repo.GetAsync(id).ConfigureAwait(false);
            return result?.ToContract();
        }

        public async Task<List<StrTransaction>?> GetAsync()
        {
            var result = await _repo.GetAsync().ConfigureAwait(false);
            return result?.ToContract();
        }

        public async Task<StrTransaction> CreateAsync(StrTransaction contract)
        ...
        public async Task<StrTransaction?> UpdateAsync(StrTransaction contract)
        {
        ...
        public async Task<StrTransaction> UploadAsync(StrTransaction contract)
        {
        ...
        public async Task<Bfs.Core.Contracts.QueryResponse<StrTransactionListItem>> ListAsync(Bfs.Core.Contracts.QueryRequest<StrTransactionListFilter> contractRequest)
        {
            var entityRequest = SerializationHelper.DoMapping<Bfs.Core.Contracts.QueryRequest<StrTransactionListFilter>, Bfs.Core.Data.QueryRequest<Data.StrTransactionListFilter>>(contractRequest);

            var entityResult = await _list.GetAsync(entityRequest).ConfigureAwait(false);
            var mappedResult = SerializationHelper.DoMapping<Bfs.Core.Data.QueryResponse<Data.StrTransactionListItem>, Bfs.Core.Contracts.QueryResponse<StrTransactionListItem>>(entityResult);

            return mappedResult ?? new Bfs.Core.Contracts.QueryResponse<StrTransactionListItem> { Items = new List<StrTransactionListItem>(), TotalItems = 0, TotalPages = 0 };
        }
