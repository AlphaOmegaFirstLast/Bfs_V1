...
{
    public class StrProductService : IStrProductService
    {
        private readonly IStrProductRepository _repo;
        private readonly IStrProductList _list;
        public StrProductService(IStrProductRepository repo, IStrProductList list)
        {
            _repo = repo;
            _list = list;
        }

        public async Task<StrProduct?> GetAsync(long id)
        {
            var result = await _repo.GetAsync(id).ConfigureAwait(false);
            return result?.ToContract();
        }

        public async Task<List<StrProduct>?> GetAsync()
        {
            var result = await _repo.GetAsync().ConfigureAwait(false);
            return result?.ToContract();
        }

        public async Task<StrProduct> CreateAsync(StrProduct contract)
        ...
        public async Task<StrProduct?> UpdateAsync(StrProduct contract)
        {
        ...
        public async Task<StrProduct> UploadAsync(StrProduct contract)
        {
        ...
        public async Task<Bfs.Core.Contracts.QueryResponse<StrProductListItem>> ListAsync(Bfs.Core.Contracts.QueryRequest<StrProductListFilter> contractRequest)
        {
            var entityRequest = SerializationHelper.DoMapping<Bfs.Core.Contracts.QueryRequest<StrProductListFilter>, Bfs.Core.Data.QueryRequest<Data.StrProductListFilter>>(contractRequest);

            var entityResult = await _list.GetAsync(entityRequest).ConfigureAwait(false);
            var mappedResult = SerializationHelper.DoMapping<Bfs.Core.Data.QueryResponse<Data.StrProductListItem>, Bfs.Core.Contracts.QueryResponse<StrProductListItem>>(entityResult);

            return mappedResult ?? new Bfs.Core.Contracts.QueryResponse<StrProductListItem> { Items = new List<StrProductListItem>(), TotalItems = 0, TotalPages = 0 };
        }
