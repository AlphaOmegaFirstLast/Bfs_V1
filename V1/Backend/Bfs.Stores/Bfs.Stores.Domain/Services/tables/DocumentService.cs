...
namespace Bfs.Stores.Domain.Services
{
    public class DocumentService : IDocumentService
    {
        ...
        private readonly IDocumentList _list;
        public DocumentService(IDocumentRepository repo, IDocumentList list)
        {
            _repo = repo;
            _list = list;
        }
        ...
        public async Task<Document> CreateAsync(Document contract)

        {
            ...
            //var message = new DocumentCreatedMessage
            //{
        ...
        public async Task<Document?> UpdateAsync(Document contract)
        {
            ...
            //var message = new DocumentUpdatedMessage
            //{
        ...
        public async Task DeleteAsync(long id)
        {
            ...
            //var message = new DocumentDeletedMessage
            //{
        ...
        public async Task<Document> UploadAsync(Document contract)
        {
        ...
        public async Task<Bfs.Core.Contracts.QueryResponse<DocumentListItem>> ListAsync(Bfs.Core.Contracts.QueryRequest<DocumentListFilter> contractRequest)
        {
            var entityRequest = SerializationHelper.DoMapping<Bfs.Core.Contracts.QueryRequest<DocumentListFilter>, Bfs.Core.Data.QueryRequest<Data.DocumentListFilter>>(contractRequest);

            var entityResult = await _list.GetAsync(entityRequest).ConfigureAwait(false);
            var mappedResult = SerializationHelper.DoMapping<Bfs.Core.Data.QueryResponse<Data.DocumentListItem>, Bfs.Core.Contracts.QueryResponse<DocumentListItem>>(entityResult);

            return mappedResult ?? new Bfs.Core.Contracts.QueryResponse<DocumentListItem> { Items = new List<DocumentListItem>(), TotalItems = 0, TotalPages = 0 };
        }
