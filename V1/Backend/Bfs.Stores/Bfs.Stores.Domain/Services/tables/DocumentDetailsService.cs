...
namespace Bfs.Stores.Domain.Services
{
    public class DocumentDetailsService : IDocumentDetailsService
    {
        ...
        private readonly IDocumentDetailsList _list;
        public DocumentDetailsService(IDocumentDetailsRepository repo, IDocumentDetailsList list)
        {
            _repo = repo;
            _list = list;
        }

        public async Task<DocumentDetails?> GetAsync(long id)
        {
            var result = await _repo.GetAsync(id).ConfigureAwait(false);
            return result?.ToContract();
        }

        public async Task<List<DocumentDetails>?> GetAsync()
        {
            var result = await _repo.GetAsync().ConfigureAwait(false);
            return result?.ToContract();
        }

        public async Task<DocumentDetails> CreateAsync(DocumentDetails contract)

        {
            ...
            //var message = new DocumentDetailsCreatedMessage
            //{
        ...
        public async Task<DocumentDetails?> UpdateAsync(DocumentDetails contract)
        {
            ...
            //var message = new DocumentDetailsUpdatedMessage
            //{
        ...
        public async Task DeleteAsync(long id)
        {
            ...
            //var message = new DocumentDetailsDeletedMessage
            //{
        ...
        public async Task<DocumentDetails> UploadAsync(DocumentDetails contract)
        {
        ...
        public async Task<Bfs.Core.Contracts.QueryResponse<DocumentDetailsListItem>> ListAsync(Bfs.Core.Contracts.QueryRequest<DocumentDetailsListFilter> contractRequest)
        {
            var entityRequest = SerializationHelper.DoMapping<Bfs.Core.Contracts.QueryRequest<DocumentDetailsListFilter>, Bfs.Core.Data.QueryRequest<Data.DocumentDetailsListFilter>>(contractRequest);

            var entityResult = await _list.GetAsync(entityRequest).ConfigureAwait(false);
            var mappedResult = SerializationHelper.DoMapping<Bfs.Core.Data.QueryResponse<Data.DocumentDetailsListItem>, Bfs.Core.Contracts.QueryResponse<DocumentDetailsListItem>>(entityResult);

            return mappedResult ?? new Bfs.Core.Contracts.QueryResponse<DocumentDetailsListItem> { Items = new List<DocumentDetailsListItem>(), TotalItems = 0, TotalPages = 0 };
        }
