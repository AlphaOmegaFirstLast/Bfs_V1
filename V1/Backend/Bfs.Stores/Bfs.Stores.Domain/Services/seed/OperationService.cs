...
namespace Bfs.Stores.Domain.Services
{
    public class OperationService : IOperationService
    {
        ...
        private readonly IOperationList _list;
        public OperationService(IOperationRepository repo, IOperationList list)
        {
            _repo = repo;
            _list = list;
        }
        ...
        public async Task<Operation> CreateAsync(Operation contract)

        {
            ...
            //var message = new OperationCreatedMessage
            //{
        ...
        public async Task<Operation?> UpdateAsync(Operation contract)
        {
            ...
            //var message = new OperationUpdatedMessage
            //{
        ...
        public async Task DeleteAsync(long id)
        {
            ...
            //var message = new OperationDeletedMessage
            //{
        ...
        public async Task<Operation> UploadAsync(Operation contract)
        {
        ...
        public async Task<Bfs.Core.Contracts.QueryResponse<OperationListItem>> ListAsync(Bfs.Core.Contracts.QueryRequest<OperationListFilter> contractRequest)
        {
            var entityRequest = SerializationHelper.DoMapping<Bfs.Core.Contracts.QueryRequest<OperationListFilter>, Bfs.Core.Data.QueryRequest<Data.OperationListFilter>>(contractRequest);

            var entityResult = await _list.GetAsync(entityRequest).ConfigureAwait(false);
            var mappedResult = SerializationHelper.DoMapping<Bfs.Core.Data.QueryResponse<Data.OperationListItem>, Bfs.Core.Contracts.QueryResponse<OperationListItem>>(entityResult);

            return mappedResult ?? new Bfs.Core.Contracts.QueryResponse<OperationListItem> { Items = new List<OperationListItem>(), TotalItems = 0, TotalPages = 0 };
        }
