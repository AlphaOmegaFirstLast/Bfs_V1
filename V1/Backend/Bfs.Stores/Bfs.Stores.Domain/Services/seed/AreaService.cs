...
namespace Bfs.Stores.Domain.Services
{
    public class AreaService : IAreaService
    {
        ...
        private readonly IAreaList _list;
        public AreaService(IAreaRepository repo, IAreaList list)
        {
            _repo = repo;
            _list = list;
        }

        public async Task<Area?> GetAsync(long id)
        {
        ...
        public async Task<Area> CreateAsync(Area contract)

        {
            ...
            //var message = new AreaCreatedMessage
            //{
        ...
        public async Task<Area?> UpdateAsync(Area contract)
        {
            ...
            //var message = new AreaUpdatedMessage
            //{
        ...
        public async Task DeleteAsync(long id)
        {
            ...
            //var message = new AreaDeletedMessage
            //{
        ...
        public async Task<Area> UploadAsync(Area contract)
        {
        ...
        public async Task<Bfs.Core.Contracts.QueryResponse<AreaListItem>> ListAsync(Bfs.Core.Contracts.QueryRequest<AreaListFilter> contractRequest)
        {
            var entityRequest = SerializationHelper.DoMapping<Bfs.Core.Contracts.QueryRequest<AreaListFilter>, Bfs.Core.Data.QueryRequest<Data.AreaListFilter>>(contractRequest);

            var entityResult = await _list.GetAsync(entityRequest).ConfigureAwait(false);
            var mappedResult = SerializationHelper.DoMapping<Bfs.Core.Data.QueryResponse<Data.AreaListItem>, Bfs.Core.Contracts.QueryResponse<AreaListItem>>(entityResult);

            return mappedResult ?? new Bfs.Core.Contracts.QueryResponse<AreaListItem> { Items = new List<AreaListItem>(), TotalItems = 0, TotalPages = 0 };
        }
