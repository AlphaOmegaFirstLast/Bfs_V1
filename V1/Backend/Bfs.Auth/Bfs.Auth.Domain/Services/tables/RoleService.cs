...
namespace Bfs.Auth.Domain.Services
{
    public class RoleService : IRoleService
    {
        ...
        private readonly IRoleList _list;
        public RoleService(IRoleRepository repo, IRoleList list)
        {
            _repo = repo;
            _list = list;
        }
        ...
        public async Task<Role> CreateAsync(Role contract)
        ...
        public async Task<Role?> UpdateAsync(Role contract)
        {
        ...
        public async Task<Role> UploadAsync(Role contract)
        {
        ...
        public async Task<Bfs.Core.Contracts.QueryResponse<RoleListItem>> ListAsync(Bfs.Core.Contracts.QueryRequest<RoleListFilter> contractRequest)
        {
            var entityRequest = SerializationHelper.DoMapping<Bfs.Core.Contracts.QueryRequest<RoleListFilter>, Bfs.Core.Data.QueryRequest<Data.RoleListFilter>>(contractRequest);

            var entityResult = await _list.GetAsync(entityRequest).ConfigureAwait(false);
            var mappedResult = SerializationHelper.DoMapping<Bfs.Core.Data.QueryResponse<Data.RoleListItem>, Bfs.Core.Contracts.QueryResponse<RoleListItem>>(entityResult);

            return mappedResult ?? new Bfs.Core.Contracts.QueryResponse<RoleListItem> { Items = new List<RoleListItem>(), TotalItems = 0, TotalPages = 0 };
        }
