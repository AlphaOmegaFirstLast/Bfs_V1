...
namespace Bfs.Auth.Domain.Services
{
    ...
    {
        private readonly IAuthRoleRepository _repo;
        private readonly IAuthRoleList _list;
        public AuthRoleService(IAuthRoleRepository repo, IAuthRoleList list)
        {
            _repo = repo;
            _list = list;
        }

        public async Task<AuthRole?> GetAsync(long id)
        {
            var result = await _repo.GetAsync(id).ConfigureAwait(false);
            return result?.ToContract();
        }

        public async Task<List<AuthRole>?> GetAsync()
        {
            var result = await _repo.GetAsync().ConfigureAwait(false);
            return result?.ToContract();
        }

        public async Task<AuthRole> CreateAsync(AuthRole contract)
        ...
        public async Task<AuthRole?> UpdateAsync(AuthRole contract)
        {
        ...
        public async Task<AuthRole> UploadAsync(AuthRole contract)
        {
        ...
        public async Task<Bfs.Core.Contracts.QueryResponse<AuthRoleListItem>> ListAsync(Bfs.Core.Contracts.QueryRequest<AuthRoleListFilter> contractRequest)
        {
            var entityRequest = SerializationHelper.DoMapping<Bfs.Core.Contracts.QueryRequest<AuthRoleListFilter>, Bfs.Core.Data.QueryRequest<Data.AuthRoleListFilter>>(contractRequest);

            var entityResult = await _list.GetAsync(entityRequest).ConfigureAwait(false);
            var mappedResult = SerializationHelper.DoMapping<Bfs.Core.Data.QueryResponse<Data.AuthRoleListItem>, Bfs.Core.Contracts.QueryResponse<AuthRoleListItem>>(entityResult);

            return mappedResult ?? new Bfs.Core.Contracts.QueryResponse<AuthRoleListItem> { Items = new List<AuthRoleListItem>(), TotalItems = 0, TotalPages = 0 };
        }
