...
namespace Bfs.Auth.Domain.Services
{
    public class AuthUserService : IAuthUserService
    {
        ...
        private readonly IAuthUserList _list;
        public AuthUserService(IAuthUserRepository repo, IAuthUserList list)
        {
            _repo = repo;
            _list = list;
        }

        public async Task<AuthUser?> GetAsync(long id)
        {
            var result = await _repo.GetAsync(id).ConfigureAwait(false);
            return result?.ToContract();
        }

        public async Task<List<AuthUser>?> GetAsync()
        {
            var result = await _repo.GetAsync().ConfigureAwait(false);
            return result?.ToContract();
        }

        public async Task<AuthUser> CreateAsync(AuthUser contract)
        ...
        public async Task<AuthUser?> UpdateAsync(AuthUser contract)
        {
        ...
        public async Task<AuthUser> UploadAsync(AuthUser contract)
        {
        ...
        public async Task<Bfs.Core.Contracts.QueryResponse<AuthUserListItem>> ListAsync(Bfs.Core.Contracts.QueryRequest<AuthUserListFilter> contractRequest)
        {
            var entityRequest = SerializationHelper.DoMapping<Bfs.Core.Contracts.QueryRequest<AuthUserListFilter>, Bfs.Core.Data.QueryRequest<Data.AuthUserListFilter>>(contractRequest);

            var entityResult = await _list.GetAsync(entityRequest).ConfigureAwait(false);
            var mappedResult = SerializationHelper.DoMapping<Bfs.Core.Data.QueryResponse<Data.AuthUserListItem>, Bfs.Core.Contracts.QueryResponse<AuthUserListItem>>(entityResult);

            return mappedResult ?? new Bfs.Core.Contracts.QueryResponse<AuthUserListItem> { Items = new List<AuthUserListItem>(), TotalItems = 0, TotalPages = 0 };
        }
