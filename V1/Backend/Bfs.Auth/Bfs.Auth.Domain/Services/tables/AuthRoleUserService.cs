...
{
    public class AuthRoleUserService : IAuthRoleUserService
    {
        private readonly IAuthRoleUserRepository _repo;
        private readonly IAuthRoleUserList _list;
        public AuthRoleUserService(IAuthRoleUserRepository repo, IAuthRoleUserList list)
        {
            _repo = repo;
            _list = list;
        }

        public async Task<AuthRoleUser?> GetAsync(long id)
        {
            var result = await _repo.GetAsync(id).ConfigureAwait(false);
            return result?.ToContract();
        }

        public async Task<List<AuthRoleUser>?> GetAsync()
        {
            var result = await _repo.GetAsync().ConfigureAwait(false);
            return result?.ToContract();
        }

        public async Task<AuthRoleUser> CreateAsync(AuthRoleUser contract)
        ...
        public async Task<AuthRoleUser?> UpdateAsync(AuthRoleUser contract)
        {
        ...
        public async Task<AuthRoleUser> UploadAsync(AuthRoleUser contract)
        {
        ...
        public async Task<Bfs.Core.Contracts.QueryResponse<AuthRoleUserListItem>> ListAsync(Bfs.Core.Contracts.QueryRequest<AuthRoleUserListFilter> contractRequest)
        {
            var entityRequest = SerializationHelper.DoMapping<Bfs.Core.Contracts.QueryRequest<AuthRoleUserListFilter>, Bfs.Core.Data.QueryRequest<Data.AuthRoleUserListFilter>>(contractRequest);

            var entityResult = await _list.GetAsync(entityRequest).ConfigureAwait(false);
            var mappedResult = SerializationHelper.DoMapping<Bfs.Core.Data.QueryResponse<Data.AuthRoleUserListItem>, Bfs.Core.Contracts.QueryResponse<AuthRoleUserListItem>>(entityResult);

            return mappedResult ?? new Bfs.Core.Contracts.QueryResponse<AuthRoleUserListItem> { Items = new List<AuthRoleUserListItem>(), TotalItems = 0, TotalPages = 0 };
        }
