...
{
    public class AuthRoleAppService : IAuthRoleAppService
    {
        private readonly IAuthRoleAppRepository _repo;
        private readonly IAuthRoleAppList _list;
        public AuthRoleAppService(IAuthRoleAppRepository repo, IAuthRoleAppList list)
        {
            _repo = repo;
            _list = list;
        }

        public async Task<AuthRoleApp?> GetAsync(long id)
        {
            var result = await _repo.GetAsync(id).ConfigureAwait(false);
            return result?.ToContract();
        }

        public async Task<List<AuthRoleApp>?> GetAsync()
        {
            var result = await _repo.GetAsync().ConfigureAwait(false);
            return result?.ToContract();
        }

        public async Task<AuthRoleApp> CreateAsync(AuthRoleApp contract)
        ...
        public async Task<AuthRoleApp?> UpdateAsync(AuthRoleApp contract)
        {
        ...
        public async Task<AuthRoleApp> UploadAsync(AuthRoleApp contract)
        {
        ...
        public async Task<Bfs.Core.Contracts.QueryResponse<AuthRoleAppListItem>> ListAsync(Bfs.Core.Contracts.QueryRequest<AuthRoleAppListFilter> contractRequest)
        {
            var entityRequest = SerializationHelper.DoMapping<Bfs.Core.Contracts.QueryRequest<AuthRoleAppListFilter>, Bfs.Core.Data.QueryRequest<Data.AuthRoleAppListFilter>>(contractRequest);

            var entityResult = await _list.GetAsync(entityRequest).ConfigureAwait(false);
            var mappedResult = SerializationHelper.DoMapping<Bfs.Core.Data.QueryResponse<Data.AuthRoleAppListItem>, Bfs.Core.Contracts.QueryResponse<AuthRoleAppListItem>>(entityResult);

            return mappedResult ?? new Bfs.Core.Contracts.QueryResponse<AuthRoleAppListItem> { Items = new List<AuthRoleAppListItem>(), TotalItems = 0, TotalPages = 0 };
        }
