...
namespace Bfs.BestFit.Domain.Services
{
    public class ComponentService : IComponentService
    {
        ...
        private readonly IComponentList _list;
        public ComponentService(IComponentRepository repo, IComponentList list)
        {
            _repo = repo;
            _list = list;
        }

        public async Task<Component?> GetAsync(long id)
        {
            var result = await _repo.GetAsync(id).ConfigureAwait(false);
            return result?.ToContract();
        }

        public async Task<List<Component>?> GetAsync()
        {
            var result = await _repo.GetAsync().ConfigureAwait(false);
            return result?.ToContract();
        }

        public async Task<Component> CreateAsync(Component contract)
        ...
        public async Task<Component?> UpdateAsync(Component contract)
        {
        ...
        public async Task<Component> UploadAsync(Component contract)
        {
        ...
        public async Task<Bfs.Core.Contracts.QueryResponse<ComponentListItem>> ListAsync(Bfs.Core.Contracts.QueryRequest<ComponentListFilter> contractRequest)
        {
            var entityRequest = SerializationHelper.DoMapping<Bfs.Core.Contracts.QueryRequest<ComponentListFilter>, Bfs.Core.Data.QueryRequest<Data.ComponentListFilter>>(contractRequest);

            var entityResult = await _list.GetComponentListAsync(entityRequest).ConfigureAwait(false);
            var mappedResult = SerializationHelper.DoMapping<Bfs.Core.Data.QueryResponse<Data.ComponentListItem>, Bfs.Core.Contracts.QueryResponse<ComponentListItem>>(entityResult);

            return mappedResult ?? new Bfs.Core.Contracts.QueryResponse<ComponentListItem> { Items = new List<ComponentListItem>(), TotalItems = 0, TotalPages = 0 };
        }
