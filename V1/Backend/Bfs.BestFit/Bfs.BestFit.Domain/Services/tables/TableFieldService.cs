...
namespace Bfs.BestFit.Domain.Services
{
    ...
    {
        private readonly ITableFieldRepository _repo;
        private readonly ITableFieldList _list;
        public TableFieldService(ITableFieldRepository repo, ITableFieldList list)
        {
            _repo = repo;
            _list = list;
        }

        public async Task<TableField?> GetAsync(long id)
        {
            var result = await _repo.GetAsync(id).ConfigureAwait(false);
            return result?.ToContract();
        }

        public async Task<List<TableField>?> GetAsync()
        {
            var result = await _repo.GetAsync().ConfigureAwait(false);
            return result?.ToContract();
        }

        public async Task<TableField> CreateAsync(TableField contract)
        ...
        public async Task<TableField?> UpdateAsync(TableField contract)
        {
        ...
        public async Task<TableField> UploadAsync(TableField contract)
        {
        ...
        public async Task<Bfs.Core.Contracts.QueryResponse<TableFieldListItem>> ListAsync(Bfs.Core.Contracts.QueryRequest<TableFieldListFilter> contractRequest)
        {
            var entityRequest = SerializationHelper.DoMapping<Bfs.Core.Contracts.QueryRequest<TableFieldListFilter>, Bfs.Core.Data.QueryRequest<Data.TableFieldListFilter>>(contractRequest);

            var entityResult = await _list.GetTableFieldListAsync(entityRequest).ConfigureAwait(false);
            var mappedResult = SerializationHelper.DoMapping<Bfs.Core.Data.QueryResponse<Data.TableFieldListItem>, Bfs.Core.Contracts.QueryResponse<TableFieldListItem>>(entityResult);

            return mappedResult ?? new Bfs.Core.Contracts.QueryResponse<TableFieldListItem> { Items = new List<TableFieldListItem>(), TotalItems = 0, TotalPages = 0 };
        }
