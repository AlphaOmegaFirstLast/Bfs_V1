...
{
    public class BfsClientSystemService : IBfsClientSystemService
    {
        private readonly IBfsClientSystemRepository _repo;
        private readonly IBfsClientSystemList _list;
        public BfsClientSystemService(IBfsClientSystemRepository repo, IBfsClientSystemList list)
        {
            _repo = repo;
            _list = list;
        }

        public async Task<BfsClientSystem?> GetAsync(long id)
        {
            var result = await _repo.GetAsync(id).ConfigureAwait(false);
            return result?.ToContract();
        }

        public async Task<List<BfsClientSystem>?> GetAsync()
        {
            var result = await _repo.GetAsync().ConfigureAwait(false);
            return result?.ToContract();
        }

        public async Task<BfsClientSystem> CreateAsync(BfsClientSystem contract)
        ...
        public async Task<BfsClientSystem?> UpdateAsync(BfsClientSystem contract)
        {
        ...
        public async Task<BfsClientSystem> UploadAsync(BfsClientSystem contract)
        {
        ...
        public async Task<Bfs.Core.Contracts.QueryResponse<BfsClientSystemListItem>> ListAsync(Bfs.Core.Contracts.QueryRequest<BfsClientSystemListFilter> contractRequest)
        {
            var entityRequest = SerializationHelper.DoMapping<Bfs.Core.Contracts.QueryRequest<BfsClientSystemListFilter>, Bfs.Core.Data.QueryRequest<Data.BfsClientSystemListFilter>>(contractRequest);

            var entityResult = await _list.GetAsync(entityRequest).ConfigureAwait(false);
            var mappedResult = SerializationHelper.DoMapping<Bfs.Core.Data.QueryResponse<Data.BfsClientSystemListItem>, Bfs.Core.Contracts.QueryResponse<BfsClientSystemListItem>>(entityResult);

            return mappedResult ?? new Bfs.Core.Contracts.QueryResponse<BfsClientSystemListItem> { Items = new List<BfsClientSystemListItem>(), TotalItems = 0, TotalPages = 0 };
        }
