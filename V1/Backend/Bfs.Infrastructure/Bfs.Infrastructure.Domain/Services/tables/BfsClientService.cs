...
{
    public class BfsClientService : IBfsClientService
    {
        private readonly IBfsClientRepository _repo;
        private readonly IBfsClientList _list;
        public BfsClientService(IBfsClientRepository repo, IBfsClientList list)
        {
            _repo = repo;
            _list = list;
        }

        public async Task<BfsClient?> GetAsync(long id)
        {
            var result = await _repo.GetAsync(id).ConfigureAwait(false);
            return result?.ToContract();
        }

        public async Task<List<BfsClient>?> GetAsync()
        {
            var result = await _repo.GetAsync().ConfigureAwait(false);
            return result?.ToContract();
        }

        public async Task<BfsClient> CreateAsync(BfsClient contract)
        ...
        public async Task<BfsClient?> UpdateAsync(BfsClient contract)
        {
        ...
        public async Task<BfsClient> UploadAsync(BfsClient contract)
        {
        ...
        public async Task<Bfs.Core.Contracts.QueryResponse<BfsClientListItem>> ListAsync(Bfs.Core.Contracts.QueryRequest<BfsClientListFilter> contractRequest)
        {
            var entityRequest = SerializationHelper.DoMapping<Bfs.Core.Contracts.QueryRequest<BfsClientListFilter>, Bfs.Core.Data.QueryRequest<Data.BfsClientListFilter>>(contractRequest);

            var entityResult = await _list.GetAsync(entityRequest).ConfigureAwait(false);
            var mappedResult = SerializationHelper.DoMapping<Bfs.Core.Data.QueryResponse<Data.BfsClientListItem>, Bfs.Core.Contracts.QueryResponse<BfsClientListItem>>(entityResult);

            return mappedResult ?? new Bfs.Core.Contracts.QueryResponse<BfsClientListItem> { Items = new List<BfsClientListItem>(), TotalItems = 0, TotalPages = 0 };
        }
