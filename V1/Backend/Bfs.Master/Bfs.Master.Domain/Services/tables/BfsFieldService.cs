...
{
    public class BfsFieldService : IBfsFieldService
    {
        private readonly IBfsFieldRepository _repo;
        private readonly IBfsFieldList _list;
        public BfsFieldService(IBfsFieldRepository repo, IBfsFieldList list)
        {
            _repo = repo;
            _list = list;
        }

        public async Task<BfsField?> GetAsync(long id)
        {
            var result = await _repo.GetAsync(id).ConfigureAwait(false);
            return result?.ToContract();
        }

        public async Task<List<BfsField>?> GetAsync()
        {
            var result = await _repo.GetAsync().ConfigureAwait(false);
            return result?.ToContract();
        }

        public async Task<BfsField> CreateAsync(BfsField contract)

        {
            ...
            //var message = new BfsFieldCreatedMessage
            //{
        ...
        public async Task<BfsField?> UpdateAsync(BfsField contract)
        {
            ...
            //var message = new BfsFieldUpdatedMessage
            //{
        ...
        public async Task DeleteAsync(long id)
        {
            ...
            //var message = new BfsFieldDeletedMessage
            //{
        ...
        public async Task<BfsField> UploadAsync(BfsField contract)
        {
        ...
        public async Task<Bfs.Core.Contracts.QueryResponse<BfsFieldListItem>> ListAsync(Bfs.Core.Contracts.QueryRequest<BfsFieldListFilter> contractRequest)
        {
            var entityRequest = SerializationHelper.DoMapping<Bfs.Core.Contracts.QueryRequest<BfsFieldListFilter>, Bfs.Core.Data.QueryRequest<Data.BfsFieldListFilter>>(contractRequest);

            var entityResult = await _list.GetAsync(entityRequest).ConfigureAwait(false);
            var mappedResult = SerializationHelper.DoMapping<Bfs.Core.Data.QueryResponse<Data.BfsFieldListItem>, Bfs.Core.Contracts.QueryResponse<BfsFieldListItem>>(entityResult);

            return mappedResult ?? new Bfs.Core.Contracts.QueryResponse<BfsFieldListItem> { Items = new List<BfsFieldListItem>(), TotalItems = 0, TotalPages = 0 };
        }
