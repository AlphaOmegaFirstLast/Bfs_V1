...
{
    public class ActionLocationService : IActionLocationService
    {
        private readonly IActionLocationRepository _repo;
        private readonly IActionLocationList _list;
        public ActionLocationService(IActionLocationRepository repo, IActionLocationList list)
        {
            _repo = repo;
            _list = list;
        }

        public async Task<ActionLocation?> GetAsync(long id)
        {
            var result = await _repo.GetAsync(id).ConfigureAwait(false);
            return result?.ToContract();
        }

        public async Task<List<ActionLocation>?> GetAsync()
        {
            var result = await _repo.GetAsync().ConfigureAwait(false);
            return result?.ToContract();
        }

        public async Task<ActionLocation> CreateAsync(ActionLocation contract)
        ...
        public async Task<ActionLocation?> UpdateAsync(ActionLocation contract)
        {
        ...
        public async Task<ActionLocation> UploadAsync(ActionLocation contract)
        {
        ...
        public async Task<Bfs.Core.Contracts.QueryResponse<ActionLocationListItem>> ListAsync(Bfs.Core.Contracts.QueryRequest<ActionLocationListFilter> contractRequest)
        {
            var entityRequest = SerializationHelper.DoMapping<Bfs.Core.Contracts.QueryRequest<ActionLocationListFilter>, Bfs.Core.Data.QueryRequest<Data.ActionLocationListFilter>>(contractRequest);

            var entityResult = await _list.GetAsync(entityRequest).ConfigureAwait(false);
            var mappedResult = SerializationHelper.DoMapping<Bfs.Core.Data.QueryResponse<Data.ActionLocationListItem>, Bfs.Core.Contracts.QueryResponse<ActionLocationListItem>>(entityResult);

            return mappedResult ?? new Bfs.Core.Contracts.QueryResponse<ActionLocationListItem> { Items = new List<ActionLocationListItem>(), TotalItems = 0, TotalPages = 0 };
        }
