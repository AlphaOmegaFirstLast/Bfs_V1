...
namespace Bfs.Master.Domain.Services
{
    ...
    {
        private readonly IActionTypeRepository _repo;
        private readonly IActionTypeList _list;
        public ActionTypeService(IActionTypeRepository repo, IActionTypeList list)
        {
            _repo = repo;
            _list = list;
        }

        public async Task<ActionType?> GetAsync(long id)
        {
            var result = await _repo.GetAsync(id).ConfigureAwait(false);
            return result?.ToContract();
        }

        public async Task<List<ActionType>?> GetAsync()
        {
            var result = await _repo.GetAsync().ConfigureAwait(false);
            return result?.ToContract();
        }

        public async Task<ActionType> CreateAsync(ActionType contract)
        ...
        public async Task<ActionType?> UpdateAsync(ActionType contract)
        {
        ...
        public async Task<ActionType> UploadAsync(ActionType contract)
        {
        ...
        public async Task<Bfs.Core.Contracts.QueryResponse<ActionTypeListItem>> ListAsync(Bfs.Core.Contracts.QueryRequest<ActionTypeListFilter> contractRequest)
        {
            var entityRequest = SerializationHelper.DoMapping<Bfs.Core.Contracts.QueryRequest<ActionTypeListFilter>, Bfs.Core.Data.QueryRequest<Data.ActionTypeListFilter>>(contractRequest);

            var entityResult = await _list.GetAsync(entityRequest).ConfigureAwait(false);
            var mappedResult = SerializationHelper.DoMapping<Bfs.Core.Data.QueryResponse<Data.ActionTypeListItem>, Bfs.Core.Contracts.QueryResponse<ActionTypeListItem>>(entityResult);

            return mappedResult ?? new Bfs.Core.Contracts.QueryResponse<ActionTypeListItem> { Items = new List<ActionTypeListItem>(), TotalItems = 0, TotalPages = 0 };
        }
