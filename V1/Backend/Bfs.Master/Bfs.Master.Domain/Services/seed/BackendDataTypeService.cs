...
{
    public class BackendDataTypeService : IBackendDataTypeService
    {
        private readonly IBackendDataTypeRepository _repo;
        private readonly IBackendDataTypeList _list;
        public BackendDataTypeService(IBackendDataTypeRepository repo, IBackendDataTypeList list)
        {
            _repo = repo;
            _list = list;
        }

        public async Task<BackendDataType?> GetAsync(long id)
        {
            var result = await _repo.GetAsync(id).ConfigureAwait(false);
            return result?.ToContract();
        }

        public async Task<List<BackendDataType>?> GetAsync()
        {
            var result = await _repo.GetAsync().ConfigureAwait(false);
            return result?.ToContract();
        }

        public async Task<BackendDataType> CreateAsync(BackendDataType contract)
        ...
        public async Task<BackendDataType?> UpdateAsync(BackendDataType contract)
        {
        ...
        public async Task<BackendDataType> UploadAsync(BackendDataType contract)
        {
        ...
        public async Task<Bfs.Core.Contracts.QueryResponse<BackendDataTypeListItem>> ListAsync(Bfs.Core.Contracts.QueryRequest<BackendDataTypeListFilter> contractRequest)
        {
            var entityRequest = SerializationHelper.DoMapping<Bfs.Core.Contracts.QueryRequest<BackendDataTypeListFilter>, Bfs.Core.Data.QueryRequest<Data.BackendDataTypeListFilter>>(contractRequest);

            var entityResult = await _list.GetAsync(entityRequest).ConfigureAwait(false);
            var mappedResult = SerializationHelper.DoMapping<Bfs.Core.Data.QueryResponse<Data.BackendDataTypeListItem>, Bfs.Core.Contracts.QueryResponse<BackendDataTypeListItem>>(entityResult);

            return mappedResult ?? new Bfs.Core.Contracts.QueryResponse<BackendDataTypeListItem> { Items = new List<BackendDataTypeListItem>(), TotalItems = 0, TotalPages = 0 };
        }
