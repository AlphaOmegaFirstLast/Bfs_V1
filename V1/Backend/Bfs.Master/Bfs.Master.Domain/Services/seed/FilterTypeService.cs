...
namespace Bfs.Master.Domain.Services
{
    ...
    {
        private readonly IFilterTypeRepository _repo;
        private readonly IFilterTypeList _list;
        public FilterTypeService(IFilterTypeRepository repo, IFilterTypeList list)
        {
            _repo = repo;
            _list = list;
        }

        public async Task<FilterType?> GetAsync(long id)
        {
            var result = await _repo.GetAsync(id).ConfigureAwait(false);
            return result?.ToContract();
        }

        public async Task<List<FilterType>?> GetAsync()
        {
            var result = await _repo.GetAsync().ConfigureAwait(false);
            return result?.ToContract();
        }

        public async Task<FilterType> CreateAsync(FilterType contract)
        ...
        public async Task<FilterType?> UpdateAsync(FilterType contract)
        {
        ...
        public async Task<FilterType> UploadAsync(FilterType contract)
        {
        ...
        public async Task<Bfs.Core.Contracts.QueryResponse<FilterTypeListItem>> ListAsync(Bfs.Core.Contracts.QueryRequest<FilterTypeListFilter> contractRequest)
        {
            var entityRequest = SerializationHelper.DoMapping<Bfs.Core.Contracts.QueryRequest<FilterTypeListFilter>, Bfs.Core.Data.QueryRequest<Data.FilterTypeListFilter>>(contractRequest);

            var entityResult = await _list.GetAsync(entityRequest).ConfigureAwait(false);
            var mappedResult = SerializationHelper.DoMapping<Bfs.Core.Data.QueryResponse<Data.FilterTypeListItem>, Bfs.Core.Contracts.QueryResponse<FilterTypeListItem>>(entityResult);

            return mappedResult ?? new Bfs.Core.Contracts.QueryResponse<FilterTypeListItem> { Items = new List<FilterTypeListItem>(), TotalItems = 0, TotalPages = 0 };
        }
