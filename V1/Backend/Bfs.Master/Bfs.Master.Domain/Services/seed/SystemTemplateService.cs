...
{
    public class SystemTemplateService : ISystemTemplateService
    {
        private readonly ISystemTemplateRepository _repo;
        private readonly ISystemTemplateList _list;
        public SystemTemplateService(ISystemTemplateRepository repo, ISystemTemplateList list)
        {
            _repo = repo;
            _list = list;
        }

        public async Task<SystemTemplate?> GetAsync(long id)
        {
            var result = await _repo.GetAsync(id).ConfigureAwait(false);
            return result?.ToContract();
        }

        public async Task<List<SystemTemplate>?> GetAsync()
        {
            var result = await _repo.GetAsync().ConfigureAwait(false);
            return result?.ToContract();
        }

        public async Task<SystemTemplate> CreateAsync(SystemTemplate contract)
        ...
        public async Task<SystemTemplate?> UpdateAsync(SystemTemplate contract)
        {
        ...
        public async Task<SystemTemplate> UploadAsync(SystemTemplate contract)
        {
        ...
        public async Task<Bfs.Core.Contracts.QueryResponse<SystemTemplateListItem>> ListAsync(Bfs.Core.Contracts.QueryRequest<SystemTemplateListFilter> contractRequest)
        {
            var entityRequest = SerializationHelper.DoMapping<Bfs.Core.Contracts.QueryRequest<SystemTemplateListFilter>, Bfs.Core.Data.QueryRequest<Data.SystemTemplateListFilter>>(contractRequest);

            var entityResult = await _list.GetAsync(entityRequest).ConfigureAwait(false);
            var mappedResult = SerializationHelper.DoMapping<Bfs.Core.Data.QueryResponse<Data.SystemTemplateListItem>, Bfs.Core.Contracts.QueryResponse<SystemTemplateListItem>>(entityResult);

            return mappedResult ?? new Bfs.Core.Contracts.QueryResponse<SystemTemplateListItem> { Items = new List<SystemTemplateListItem>(), TotalItems = 0, TotalPages = 0 };
        }
