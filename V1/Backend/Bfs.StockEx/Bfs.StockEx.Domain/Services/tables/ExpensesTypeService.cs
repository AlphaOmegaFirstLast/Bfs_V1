...
namespace Bfs.StockEx.Domain.Services
{
    ...
    {
        private readonly IExpensesTypeRepository _repo;
        private readonly IExpensesTypeList _list;
        public ExpensesTypeService(IExpensesTypeRepository repo, IExpensesTypeList list)
        {
            _repo = repo;
            _list = list;
        }

        public async Task<ExpensesType?> GetAsync(long id)
        {
            var result = await _repo.GetAsync(id).ConfigureAwait(false);
            return result?.ToContract();
        }

        public async Task<List<ExpensesType>?> GetAsync()
        {
            var result = await _repo.GetAsync().ConfigureAwait(false);
            return result?.ToContract();
        }

        public async Task<ExpensesType> CreateAsync(ExpensesType contract)

        {
            ...
            //var message = new ExpensesTypeCreatedMessage
            //{
        ...
        public async Task<ExpensesType?> UpdateAsync(ExpensesType contract)
        {
            ...
            //var message = new ExpensesTypeUpdatedMessage
            //{
        ...
        public async Task DeleteAsync(long id)
        {
            ...
            //var message = new ExpensesTypeDeletedMessage
            //{
        ...
        public async Task<ExpensesType> UploadAsync(ExpensesType contract)
        {
        ...
        public async Task<Bfs.Core.Contracts.QueryResponse<ExpensesTypeListItem>> ListAsync(Bfs.Core.Contracts.QueryRequest<ExpensesTypeListFilter> contractRequest)
        {
            var entityRequest = SerializationHelper.DoMapping<Bfs.Core.Contracts.QueryRequest<ExpensesTypeListFilter>, Bfs.Core.Data.QueryRequest<Data.ExpensesTypeListFilter>>(contractRequest);

            var entityResult = await _list.GetAsync(entityRequest).ConfigureAwait(false);
            var mappedResult = SerializationHelper.DoMapping<Bfs.Core.Data.QueryResponse<Data.ExpensesTypeListItem>, Bfs.Core.Contracts.QueryResponse<ExpensesTypeListItem>>(entityResult);

            return mappedResult ?? new Bfs.Core.Contracts.QueryResponse<ExpensesTypeListItem> { Items = new List<ExpensesTypeListItem>(), TotalItems = 0, TotalPages = 0 };
        }
