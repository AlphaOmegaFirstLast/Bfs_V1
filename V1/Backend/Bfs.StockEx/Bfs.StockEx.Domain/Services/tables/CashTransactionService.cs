...
namespace Bfs.StockEx.Domain.Services
{
    public class CashTransactionService : ICashTransactionService
    {
        ...
        private readonly ICashTransactionList _list;
        public CashTransactionService(ICashTransactionRepository repo, ICashTransactionList list)
        {
            _repo = repo;
            _list = list;
        }

        public async Task<CashTransaction?> GetAsync(long id)
        {
            var result = await _repo.GetAsync(id).ConfigureAwait(false);
            return result?.ToContract();
        }

        public async Task<List<CashTransaction>?> GetAsync()
        {
            var result = await _repo.GetAsync().ConfigureAwait(false);
            return result?.ToContract();
        }

        public async Task<CashTransaction> CreateAsync(CashTransaction contract)

        {
            ...
            //var message = new CashTransactionCreatedMessage
            //{
        ...
        public async Task<CashTransaction?> UpdateAsync(CashTransaction contract)
        {
            ...
            //var message = new CashTransactionUpdatedMessage
            //{
        ...
        public async Task DeleteAsync(long id)
        {
            ...
            //var message = new CashTransactionDeletedMessage
            //{
        ...
        public async Task<CashTransaction> UploadAsync(CashTransaction contract)
        {
        ...
        public async Task<Bfs.Core.Contracts.QueryResponse<CashTransactionListItem>> ListAsync(Bfs.Core.Contracts.QueryRequest<CashTransactionListFilter> contractRequest)
        {
            var entityRequest = SerializationHelper.DoMapping<Bfs.Core.Contracts.QueryRequest<CashTransactionListFilter>, Bfs.Core.Data.QueryRequest<Data.CashTransactionListFilter>>(contractRequest);

            var entityResult = await _list.GetAsync(entityRequest).ConfigureAwait(false);
            var mappedResult = SerializationHelper.DoMapping<Bfs.Core.Data.QueryResponse<Data.CashTransactionListItem>, Bfs.Core.Contracts.QueryResponse<CashTransactionListItem>>(entityResult);

            return mappedResult ?? new Bfs.Core.Contracts.QueryResponse<CashTransactionListItem> { Items = new List<CashTransactionListItem>(), TotalItems = 0, TotalPages = 0 };
        }
