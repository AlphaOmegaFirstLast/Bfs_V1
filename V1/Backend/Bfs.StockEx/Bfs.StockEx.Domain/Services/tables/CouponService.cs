...
namespace Bfs.StockEx.Domain.Services
{
    public class CouponService : ICouponService
    {
        ...
        private readonly ICouponList _list;
        public CouponService(ICouponRepository repo, ICouponList list)
        {
            _repo = repo;
            _list = list;
        }
        ...
        public async Task<Coupon> CreateAsync(Coupon contract)

        {
            ...
            //var message = new CouponCreatedMessage
            //{
        ...
        public async Task<Coupon?> UpdateAsync(Coupon contract)
        {
            ...
            //var message = new CouponUpdatedMessage
            //{
        ...
        public async Task DeleteAsync(long id)
        {
            ...
            //var message = new CouponDeletedMessage
            //{
        ...
        public async Task<Coupon> UploadAsync(Coupon contract)
        {
        ...
        public async Task<Bfs.Core.Contracts.QueryResponse<CouponListItem>> ListAsync(Bfs.Core.Contracts.QueryRequest<CouponListFilter> contractRequest)
        {
            var entityRequest = SerializationHelper.DoMapping<Bfs.Core.Contracts.QueryRequest<CouponListFilter>, Bfs.Core.Data.QueryRequest<Data.CouponListFilter>>(contractRequest);

            var entityResult = await _list.GetAsync(entityRequest).ConfigureAwait(false);
            var mappedResult = SerializationHelper.DoMapping<Bfs.Core.Data.QueryResponse<Data.CouponListItem>, Bfs.Core.Contracts.QueryResponse<CouponListItem>>(entityResult);

            return mappedResult ?? new Bfs.Core.Contracts.QueryResponse<CouponListItem> { Items = new List<CouponListItem>(), TotalItems = 0, TotalPages = 0 };
        }
