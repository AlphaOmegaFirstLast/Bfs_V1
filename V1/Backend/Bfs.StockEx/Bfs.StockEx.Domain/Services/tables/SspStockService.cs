...
{
    public class SspStockService : ISspStockService
    {
        private readonly ISspStockRepository _repo;
        private readonly ISspStockList _list;
        public SspStockService(ISspStockRepository repo, ISspStockList list)
        {
            _repo = repo;
            _list = list;
        }

        public async Task<SspStock?> GetAsync(long id)
        {
            var result = await _repo.GetAsync(id).ConfigureAwait(false);
            return result?.ToContract();
        }

        public async Task<List<SspStock>?> GetAsync()
        {
            var result = await _repo.GetAsync().ConfigureAwait(false);
            return result?.ToContract();
        }

        public async Task<SspStock> CreateAsync(SspStock contract)

        {
            ...
            //var message = new SspStockCreatedMessage
            //{
        ...
        public async Task<SspStock?> UpdateAsync(SspStock contract)
        {
            ...
            //var message = new SspStockUpdatedMessage
            //{
        ...
        public async Task DeleteAsync(long id)
        {
            ...
            //var message = new SspStockDeletedMessage
            //{
        ...
        public async Task<SspStock> UploadAsync(SspStock contract)
        {
        ...
        public async Task<Bfs.Core.Contracts.QueryResponse<SspStockListItem>> ListAsync(Bfs.Core.Contracts.QueryRequest<SspStockListFilter> contractRequest)
        {
            var entityRequest = SerializationHelper.DoMapping<Bfs.Core.Contracts.QueryRequest<SspStockListFilter>, Bfs.Core.Data.QueryRequest<Data.SspStockListFilter>>(contractRequest);

            var entityResult = await _list.GetAsync(entityRequest).ConfigureAwait(false);
            var mappedResult = SerializationHelper.DoMapping<Bfs.Core.Data.QueryResponse<Data.SspStockListItem>, Bfs.Core.Contracts.QueryResponse<SspStockListItem>>(entityResult);

            return mappedResult ?? new Bfs.Core.Contracts.QueryResponse<SspStockListItem> { Items = new List<SspStockListItem>(), TotalItems = 0, TotalPages = 0 };
        }
