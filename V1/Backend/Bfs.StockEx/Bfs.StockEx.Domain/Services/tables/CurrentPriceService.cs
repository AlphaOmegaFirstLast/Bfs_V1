...
namespace Bfs.StockEx.Domain.Services
{
    ...
    {
        private readonly ICurrentPriceRepository _repo;
        private readonly ICurrentPriceList _list;
        public CurrentPriceService(ICurrentPriceRepository repo, ICurrentPriceList list)
        {
            _repo = repo;
            _list = list;
        }

        public async Task<CurrentPrice?> GetAsync(long id)
        {
            var result = await _repo.GetAsync(id).ConfigureAwait(false);
            return result?.ToContract();
        }

        public async Task<List<CurrentPrice>?> GetAsync()
        {
            var result = await _repo.GetAsync().ConfigureAwait(false);
            return result?.ToContract();
        }

        public async Task<CurrentPrice> CreateAsync(CurrentPrice contract)

        {
            ...
            //var message = new CurrentPriceCreatedMessage
            //{
        ...
        public async Task<CurrentPrice?> UpdateAsync(CurrentPrice contract)
        {
            ...
            //var message = new CurrentPriceUpdatedMessage
            //{
        ...
        public async Task DeleteAsync(long id)
        {
            ...
            //var message = new CurrentPriceDeletedMessage
            //{
        ...
        public async Task<CurrentPrice> UploadAsync(CurrentPrice contract)
        {
        ...
        public async Task<Bfs.Core.Contracts.QueryResponse<CurrentPriceListItem>> ListAsync(Bfs.Core.Contracts.QueryRequest<CurrentPriceListFilter> contractRequest)
        {
            var entityRequest = SerializationHelper.DoMapping<Bfs.Core.Contracts.QueryRequest<CurrentPriceListFilter>, Bfs.Core.Data.QueryRequest<Data.CurrentPriceListFilter>>(contractRequest);

            var entityResult = await _list.GetAsync(entityRequest).ConfigureAwait(false);
            var mappedResult = SerializationHelper.DoMapping<Bfs.Core.Data.QueryResponse<Data.CurrentPriceListItem>, Bfs.Core.Contracts.QueryResponse<CurrentPriceListItem>>(entityResult);

            return mappedResult ?? new Bfs.Core.Contracts.QueryResponse<CurrentPriceListItem> { Items = new List<CurrentPriceListItem>(), TotalItems = 0, TotalPages = 0 };
        }
