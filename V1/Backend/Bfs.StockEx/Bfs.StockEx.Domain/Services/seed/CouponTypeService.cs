...
namespace Bfs.StockEx.Domain.Services
{
    ...
    {
        private readonly ICouponTypeRepository _repo;
        private readonly ICouponTypeList _list;
        public CouponTypeService(ICouponTypeRepository repo, ICouponTypeList list)
        {
            _repo = repo;
            _list = list;
        }

        public async Task<CouponType?> GetAsync(long id)
        {
            var result = await _repo.GetAsync(id).ConfigureAwait(false);
            return result?.ToContract();
        }

        public async Task<List<CouponType>?> GetAsync()
        {
            var result = await _repo.GetAsync().ConfigureAwait(false);
            return result?.ToContract();
        }

        public async Task<CouponType> CreateAsync(CouponType contract)

        {
            ...
            //var message = new CouponTypeCreatedMessage
            //{
        ...
        public async Task<CouponType?> UpdateAsync(CouponType contract)
        {
            ...
            //var message = new CouponTypeUpdatedMessage
            //{
        ...
        public async Task DeleteAsync(long id)
        {
            ...
            //var message = new CouponTypeDeletedMessage
            //{
        ...
        public async Task<CouponType> UploadAsync(CouponType contract)
        {
        ...
        public async Task<Bfs.Core.Contracts.QueryResponse<CouponTypeListItem>> ListAsync(Bfs.Core.Contracts.QueryRequest<CouponTypeListFilter> contractRequest)
        {
            var entityRequest = SerializationHelper.DoMapping<Bfs.Core.Contracts.QueryRequest<CouponTypeListFilter>, Bfs.Core.Data.QueryRequest<Data.CouponTypeListFilter>>(contractRequest);

            var entityResult = await _list.GetAsync(entityRequest).ConfigureAwait(false);
            var mappedResult = SerializationHelper.DoMapping<Bfs.Core.Data.QueryResponse<Data.CouponTypeListItem>, Bfs.Core.Contracts.QueryResponse<CouponTypeListItem>>(entityResult);

            return mappedResult ?? new Bfs.Core.Contracts.QueryResponse<CouponTypeListItem> { Items = new List<CouponTypeListItem>(), TotalItems = 0, TotalPages = 0 };
        }
