export const environment = {
    config: 'production'
  , isSecurityEnabled :true 
 , identityWebOrigin: 'https://mybepozstaging.azurewebsites.net/auth/api' 
  , contentApiUrl: 'https://mybepozstagingcontentapi.azurewebsites.net/api/'
  , logoutUrl: 'https://mybepozstaging.azurewebsites.net/auth/account/logout'
  , loginUrl: 'https://mybepozstaging.azurewebsites.net/auth'
      , BestFitApiUrl: 'https://[ProductionOrigin]/api'
  , InfrastructureApiUrl: 'https://[ProductionOrigin]/api'
  , StoresApiUrl: 'https://[ProductionOrigin]/api'
  , AuthApiUrl: 'https://[ProductionOrigin]/api'
  , MasterApiUrl: 'https://[ProductionOrigin]/api'
  , StockExApiUrl: 'https://[ProductionOrigin]/api'
//Template_System_AddEnvironmentEntry
};
