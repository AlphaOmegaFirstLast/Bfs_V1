...
//---------------------- Component Specific ------------------------
import { type IDocumentDetails, type IDocumentDetailsRequest, initDocumentDetails, documentDetailsUntypedFormGroup } from './document-details.shared';
import { getDocumentDetailsActions,  initDocumentDetailsRequest } from './document-details.shared';

@Component({
    selector: 'document-details-form',
    imports: [
...
export class DocumentDetailsFormComponent extends BaseFormComponent<IDocumentDetails > implements OnInit {

    override apiUrl =  '/DocumentDetails/';
    override apiService: StoresService = inject(StoresService);
    override componentName: string = 'DocumentDetails'.toLowerCase();  // used to grab its related custom field definitions
...
public DocumentOptions: any[] = [];
    ...
    constructor(activatedRoute: ActivatedRoute) {
       ...
       this.validationForm = this.formBuilder.group(documentDetailsUntypedFormGroup(this.formBuilder)); // Use Angular Validation Controls
    ...
    //---------------------------------------------------------
    override initEntity(): IDocumentDetails  {
        return initDocumentDetails ();
    }
    //---------------------------------------------------------
...
this.isLoading.lookups = true;
        ...
        (await this.apiService.post(target,  {pageSize:50})).subscribe({
            next: (response: IQueryResponse) => {
                ...
                error: (err: any) => {
                this.isLoading.lookups = false;
                var msg = err.message || 'An error occurred while fetching Unit data.';
                this.messages.push({ text: msg, msgType: "danger" });
            }
        });
this.isLoading.lookups = true;
        ...
        (await this.apiService.post(target,  {pageSize:50})).subscribe({
            next: (response: IQueryResponse) => {
                ...
                error: (err: any) => {
                this.isLoading.lookups = false;
                var msg = err.message || 'An error occurred while fetching Document data.';
                this.messages.push({ text: msg, msgType: "danger" });
            }
        });
