...
import { type ICashTransactionWithLookup, type ICashTransactionRequest, type ICashTransactionFilter } from './cash-transaction.shared';
import { getCashTransactionActions,  initCashTransactionRequest } from './cash-transaction.shared';
import { CashTransactionFilterComponent } from './cash-transaction.filter.component'; 

@Component({
    selector: 'cash-transaction-list',     
...
export class CashTransactionListComponent         
    ...
    extends BaseReportComponent<ICashTransactionFilter, ICashTransactionWithLookup> {
    override apiService: StockExService = inject(StockExService);
    override queryRequest = {} as ICashTransactionRequest;
    override exportRequest = {} as ICashTransactionRequest;
    override downloadFileName: string = "Cash Transactions";
    ...
    constructor(modalService: NgbModal, router: Router, excelService: ExcelExportService, activatedRoute: ActivatedRoute) {
        ...
        this.addNewRecordLink = { route: "/stkx/cash-transaction/add/0", displayText: "Add New Cash Transactions" };
        this.getApiUrl = '/CashTransaction/List';
        this.uploadApiUrl = '/CashTransaction/upload';

        this.filterComponent = CashTransactionFilterComponent;
        this.queryRequest = initCashTransactionRequest();
    }
    //---------------------------------------------------------
    override render(record: IEntity, column: IQueryColumn): any {
        const value = record[column.fieldName as keyof IQueryColumn];
        switch (column.fieldName) {
            case 'sspTransactionId':
                return record['sspTransactionName']?.toString();
case 'ssPortfolioId':
