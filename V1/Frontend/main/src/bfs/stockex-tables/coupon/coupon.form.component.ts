...
//---------------------- Component Specific ------------------------
import { type ICoupon, type ICouponRequest, initCoupon, couponUntypedFormGroup } from './coupon.shared';
...
this.isLoading.lookups = true;
        ...
        (await this.apiService.post(target,  {pageSize:50})).subscribe({
            next: (response: IQueryResponse) => {
                ...
                error: (err: any) => {
                this.isLoading.lookups = false;
                var msg = err.message || 'An error occurred while fetching Coupon Type data.';
                this.messages.push({ text: msg, msgType: "danger" });
            }
        });
this.isLoading.lookups = true;
        ...
        (await this.apiService.post(target,  {pageSize:50})).subscribe({
            next: (response: IQueryResponse) => {
                ...
                error: (err: any) => {
                this.isLoading.lookups = false;
                var msg = err.message || 'An error occurred while fetching Coupon Status data.';
                this.messages.push({ text: msg, msgType: "danger" });
            }
        });
