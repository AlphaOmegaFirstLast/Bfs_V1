...
import { type IAuthRoleUserWithLookup, type IAuthRoleUserRequest, type IAuthRoleUserFilter } from './auth-role-user.shared';
import { getAuthRoleUserActions,  initAuthRoleUserRequest } from './auth-role-user.shared';
import { AuthRoleUserFilterComponent } from './auth-role-user.filter.component'; 

@Component({
    selector: 'auth-role-user-list',     
...
export class AuthRoleUserListComponent         

    extends BaseReportComponent<IAuthRoleUserFilter, IAuthRoleUserWithLookup> {
    override apiService: AuthService = inject(AuthService);
    override tokenService: TokenService = inject(TokenService);
    override queryRequest = {} as IAuthRoleUserRequest;
    override exportRequest = {} as IAuthRoleUserRequest;
    ...
    constructor(modalService: NgbModal, router: Router, excelService: ExcelExportService, activatedRoute: ActivatedRoute) {
        ...
        this.addNewRecordLink = { route: "/bfs/auth-role-user/add/0", displayText: "Add New Role - User" };
        this.getApiUrl = '/AuthRoleUser/List';
        this.uploadApiUrl = '/AuthRoleUser/upload';

        this.filterComponent = AuthRoleUserFilterComponent;
        this.queryRequest = initAuthRoleUserRequest();
    }
    //---------------------------------------------------------
    override render(record: IEntity, column: IColumns): any {
        ...
        switch (column.fieldName) {
            case 'authRoleId':
                return record['authRoleName']?.toString();
case 'authUserId':
                return record['authUserName']?.toString();
