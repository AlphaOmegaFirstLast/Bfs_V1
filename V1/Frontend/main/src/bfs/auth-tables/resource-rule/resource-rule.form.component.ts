...
//---------------------- Component Specific ------------------------
import { type IResourceRule, type IResourceRuleRequest, initResourceRule, resourceRuleUntypedFormGroup } from './resource-rule.shared';
import { getResourceRuleActions,  initResourceRuleRequest } from './resource-rule.shared';

@Component({
    selector: 'resource-rule-form',
    imports: [

    CommonModule, NgIcon, NgbPopoverModule, NgbAlertModule, FormsModule, ReactiveFormsModule, NgbDropdownModule, NgbNavModule,RouterLink],
    standalone: true,
    templateUrl: './resource-rule.form.component.html',
})
export class ResourceRuleFormComponent extends BaseFormComponent<IResourceRule > implements OnInit {

    override apiUrl =  '/ResourceRule/';
    override apiService: AuthService = inject(AuthService);
    override componentName: string = 'ResourceRule'.toLowerCase();  // used to grab its related custom field definitions
...
public BfsComponentOptions: any[] = [];
    ...
    constructor(activatedRoute: ActivatedRoute) {
       ...
       this.validationForm = this.formBuilder.group(resourceRuleUntypedFormGroup(this.formBuilder)); // Use Angular Validation Controls
    ...
    //---------------------------------------------------------
    override initEntity(): IResourceRule  {
        return initResourceRule ();
    }
    //---------------------------------------------------------
...
this.isLoading.lookups = true;
        ...
        (await this.masterService.post(target,  {pageSize:50})).subscribe({
            next: (response: IQueryResponse) => {
                ...
                error: (err: any) => {
                this.isLoading.lookups = false;
                var msg = err.message || 'An error occurred while fetching BfsComponent data.';
                this.messages.push({ text: msg, msgType: "danger" });
            }
        });
