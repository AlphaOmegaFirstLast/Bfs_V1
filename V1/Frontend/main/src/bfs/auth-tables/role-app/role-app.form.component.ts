...
//---------------------- Component Specific ------------------------
import { type IRoleApp, type IRoleAppRequest, initRoleApp, roleAppUntypedFormGroup } from './role-app.shared';
import { getRoleAppActions,  initRoleAppRequest } from './role-app.shared';

@Component({
    selector: 'role-app-form',
    imports: [
...
export class RoleAppFormComponent extends BaseFormComponent<IRoleApp > implements OnInit {

    override apiUrl =  '/RoleApp/';
    override apiService: AuthService = inject(AuthService);
    override componentName: string = 'RoleApp'.toLowerCase();  // used to grab its related custom field definitions
