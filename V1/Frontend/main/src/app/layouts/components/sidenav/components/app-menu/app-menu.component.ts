...
import {menuItems as InspiniaMenuItems} from '@layouts/components/data';
...
export class AppMenuComponent implements OnInit {
    ...
    async ngOnInit(): Promise<void> {
        this.menuItems = await this.menuGuardService.getMenuItems();

        if (environment.config.toLowerCase() == 'dev') {
           // For development purposes, include Inspinia menu items
          //  this.menuItems = this.menuItems.concat(InspiniaMenuItems);
        }
