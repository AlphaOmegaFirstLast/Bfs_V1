...
namespace Admin.App
{
    public class BfsComponentEntity: IComponentEntity
    {
        ...
        // public ReportType ReportTypeId { get; set; } = ReportType.List; //ToDo add this property to the interface if needed, but for now it's only used in some components like reports, so we can keep it in the entity for now.
    }
}