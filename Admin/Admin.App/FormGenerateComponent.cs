...
namespace CodeAdmin
{
    public partial class FormGenerateComponent : Form
    {
        ...
        private void FormGenerateComponent_Load(object sender, EventArgs e)
        {
            // init UI controls
            ...
            txtBfsRootDir.Text = _codeInfo.BfsRootDir;
            LoadSystemInfo();
        ...
        {
            var systemList = _codeInfo.SystemList;
        ...
        {
            _codeInfo.SelectedTemplateList.Clear();
        ...
        private void LoadTemplateList(List<TemplateElementType> templateElementTypeList, bool enabled = false)
        {
            ...
            templateList = _codeInfo.TemplateList.Where(x => templateElementTypeList.Contains(x.TemplateElementType)).ToList();
        ...
        private void TemplateCheckBox_Click(object sender, EventArgs e)
        {
            ...
            _codeInfo.SelectedTemplateList = _codeInfo.TemplateList.Where(x =>
        ...
        private void ClearComponentSelection()
        {
            ...
            _codeInfo.SelectedComponentList.Clear();
            _codeInfo.SetComponent(null);
        }
        ...
        private void AllCheckBox_Click(object sender, EventArgs e)
        {
            ...
            _codeInfo.SelectedComponentList = _codeInfo.ComponentList.Where(x => (DataType)x.DataTypeId == dataType).ToList();
            _codeInfo.CurrentComponent = null;

            //If only one component is selected, set it as current component. so that SetUIControls() logic enables the right buttons.
            if (_codeInfo.SelectedComponentList.Count == 1)
            {
                _codeInfo.SetComponent(_codeInfo.SelectedComponentList.First());
            }
        ...
        {
            var systemEntity = cbSystemInfo.SelectedItem == null ? null : (SystemInfoEntity)cbSystemInfo.SelectedItem;
            _codeInfo.SetSystem(systemEntity);

            // Fill Tables Grid
            SetDataGrid(_codeInfo.CurrentSystem?.Id, DataType.Tables, dataGridTables);
            // Fill reports Grid
            SetDataGrid(_codeInfo.CurrentSystem?.Id, DataType.Reports, dataGridReports);
            // Fill Seed Tables Grid
            SetDataGrid(_codeInfo.CurrentSystem?.Id, DataType.Seed, dataGridSeeds);
        }
        ...
        private void SetDataGrid(long? systemInfoId, DataType dataType, DataGridView dataGridView)
        {
            ...
            dataGridView.DataSource = _codeInfo.ComponentList.Where(x => x.BfsSystemId == systemInfoId && (DataType)x.DataTypeId == dataType).ToList();
        ...
        private void DataGridView_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (dataGridView?.Columns[e.ColumnIndex] is DataGridViewButtonColumn && e.RowIndex >= 0)
            {
                ...
                if (component != null)
                {
                    ...
                    _codeInfo.SelectedTemplateList.Clear();
                    _codeInfo.SelectedComponentList.Clear();
                    _codeInfo.SetComponent(component);
                    SetUIControls("Selected: ", _codeInfo.CurrentComponent?.Name);
        ...
        private void SetUIControls(string operation, string itemName)
        {
            ...
            var isTemplate = _codeInfo.SelectedTemplateList.Count > 0;
            var isComponent = _codeInfo.SelectedComponentList.Count == 1;
            var isAllComponent = _codeInfo.SelectedComponentList.Count > 1;
        ...
        private void btnRollBackGenerateComponent_Click(object sender, EventArgs e)
        {
            ...
            foreach (var componentTemplate in _codeInfo.SelectedTemplateList)
            {
                ...
                var currentControlsKey = GetControlsKey(_codeInfo, "Generate");
                if (lastControlsKey != currentControlsKey)
                {
                    lastControlsKey = currentControlsKey;
                    CreateControlsOfGeneratedFiles(_codeInfo.CurrentTemplate);
                }
                ...
                foreach (var control in panelItems.Controls)
                {
                    ...
                    {
                        generateItemControl.SetUp(_codeInfo);
        ...
        private void btnRollBackModifyComponent_Click(object sender, EventArgs e)
        {
            ...
            foreach (var componentTemplate in _codeInfo.SelectedTemplateList)
            {
                ...
                var currentControlsKey = GetControlsKey(_codeInfo, "Modify");
                if (lastControlsKey != currentControlsKey)
                {
                    lastControlsKey = currentControlsKey;
                    CreateControlsOfModifiedFiles(_codeInfo.CurrentTemplate);
                }
                ...
                foreach (var control in panelItems.Controls)
                {
                    ...
                    {
                        modifyItemControl.SetUp(_codeInfo);
        ...
        {
            var filteredComponents = _codeInfo.SelectedComponentList;
            foreach (var item in filteredComponents)
            {
                _codeInfo.SetComponent(item);
